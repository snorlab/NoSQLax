...
const findUsingMango = function () {
    return __awaiter(this, arguments, void 0, function* (selector = {}, options = {}, entityClass, connection, fieldMap) {
        try {
            let query = Object.assign(Object.assign({}, options), { // Spread the properties from MangoOptions
                selector });
            // add type
...
class CouchRepository {
    constructor(ds, ajvOptions, entityClass) {
        // Check if entityClass extends BaseEntity
        if (!(entityClass.prototype instanceof ActiveRecordEntity_1.default) && !(entityClass.prototype instanceof DataMapperEntity_1.default)) {
    ...
    findOne(selector_1) {
        return __awaiter(this, arguments, void 0, function* (selector, options = {}) {
            try {
                const res = yield findUsingMango(translateSelector(selector, this.fieldMap), options, this.entityClass, this.dataSource.connection, this.fieldMap);
    ...
    findAll() {
        return __awaiter(this, arguments, void 0, function* (options = {}) {
            try {
                const res = yield findUsingMango({}, options, this.entityClass, this.dataSource.connection, this.fieldMap);
                return res.docs.map((doc) => new this.entityClass(inverseTransform(doc, this.fieldMap)));
            }
            catch (err) {
                throw err;
    ...
    save(data) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (data.id) {
                    try {
                        existingDoc = yield this.find(data.id);
                    }
                    catch (error) {
                        if (!(error instanceof DocumentNotFoundError_1.DocumentNotFoundError)) {
                            throw error; // Only suppress "not found" errors
                        }
                    }
                }
                // Merge existing data with new data
                const transformedData = transformToDocumentFormat(Object.assign(Object.assign({}, existingDoc), data), this.entityClass, this.fieldMap);
                ...
                const response = yield this.dataSource.connection.insert(transformedData);
    ...
    delete(id) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                yield this.dataSource.connection.destroy(id, existingDoc.rev);
